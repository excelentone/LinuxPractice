		...
		const char* path,const char* query_string) //处理CGI模式的请求
{
	...
	//因为环境变量有上限，而GET方法传递的参数少，所以用环境变量
	//POST方法，在正文内传参，参数多，所以用管道
	...
	//把是什么方法放到环境变量里，方便子进程执行的cGI判断是什么方法。
	putenv(method_env);
...
int handler_msg(int sock)       //浏览器请求处理函数
{
	...
	printf("strlen(path)-1为：%c\n",path[strlen(path)-1]);
	//if(path[strlen(path)-1]=='/')              //判断浏览器请求的是不是目录
//	{
//		strcat(path,"index.html");//如果请求的是目录，则就把该目录下的首页返回
//	}
	printf("*************************************\n");
	struct stat st;
	printf("强侧\n");
